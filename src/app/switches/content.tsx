...
import { SwitchFilterModal } from '@/components/product/switch-filter-modal';
...
export default function SwitchesContent({ banners = [] }: { banners?: Banner[] }) {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/switches?${params.toString()}`);
      if (res.ok) {
  ...
  const handleDraftChange = useCallback(
    (df: Record<string, string[]>, dp: { min: string; max: string }) => {
      ...
      fetch(`/api/switches?${params.toString()}`, { signal: ctrl.signal })
        .then((r) => r.json())
      ...
      <div className="catalog-page">
        ...
        {loading ? (
          ...
          <div className="neo-card p-12 text-center">
            <div className="font-[family-name:var(--f-d)] text-2xl font-extrabold uppercase mb-2">No Switches Found</div>
