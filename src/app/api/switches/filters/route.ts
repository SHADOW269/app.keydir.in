...
export async function GET() {
  ...
  if (!switchesCategory) {
    return NextResponse.json({
      types: [], brands: [], vendors: [], priceRange: { min: 0, max: 100000 },
    });
  }

  const [products, vendorProducts] = await Promise.all([
    prisma.product.findMany({
      where: { categoryId: switchesCategory.id },
      select: {
    ...
    prisma.vendorProduct.findMany({
      where: { product: { categoryId: switchesCategory.id } },
  ...
  return NextResponse.json({
    types: ['linear', 'tactile', 'clicky', 'silent_linear', 'silent_tactile'],
