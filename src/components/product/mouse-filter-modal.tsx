...
interface FilterData {
  specFields: { name: string; slug: string; options: string[] }[];
  brands: { name: string; slug: string }[];
  vendors: { name: string; slug: string }[];
  priceRange: { min: number; max: number };
}

interface MouseFilterModalProps {
  open: boolean;
  onClose: () => void;
...
const STATIC_CATEGORIES: { key: string; label: string; options: string[] }[] = [
...
export function MouseFilterModal({ open, onClose, onApply, liveCount, onFilterChange }: MouseFilterModalProps) {
  ...
  const [activeCat, setActiveCat] = useState('connection');
  ...
  const categories: Category[] = useMemo(() => {
    const cats: Category[] = [];
    if (filterData) {
      for (const sf of filterData.specFields) {
        cats.push({ key: sf.slug, label: sf.name, count: getCount(sf.slug, draft) });
      }
    }
    for (const sc of STATIC_CATEGORIES) {
      cats.push({ key: sc.key, label: sc.label, count: getCount(sc.key, draft) });
    }
    if (filterData) {
  ...
  const currentGroup = useMemo(() => {
    ...
    const staticCat = STATIC_CATEGORIES.find((c) => c.key === activeCat);
    if (staticCat) return { key: staticCat.key, label: staticCat.label, options: staticCat.options.map((v) => ({ value: v, label: v })) };
    if (filterData) {
      const sf = filterData.specFields.find((f) => f.slug === activeCat);
      ...
      <div className="fm" ref={modalRef} role="dialog" aria-modal="true">
        <div className="fm-titlebar">
          <div className="fm-titlebar-left">
            ...
            <span className="fm-titlebar-text">FILTER_MOUSE.sh</span>
          </div>
        ...
        <div className="fm-footer">
          ...
          <div className="fm-footer-count">{liveCount} Mice Found</div>
