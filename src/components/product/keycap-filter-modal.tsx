...
interface KeycapFilterModalProps {
  open: boolean;
  onClose: () => void;
...
const KEYCAP_CATEGORIES: { key: string; label: string; options: string[] }[] = [
  { key: 'profile', label: 'Profile', options: ['Cherry', 'OEM', 'XDA', 'DSA', 'CSA', 'KAT', 'MT3', 'MDA', 'ASA', 'OSA'] },
  { key: 'material', label: 'Material', options: ['ABS', 'PBT', 'POM'] },
  { key: 'legend', label: 'Legend Type', options: ['Doubleshot', 'Dye Sub', 'Reverse Dye Sub', 'UV Printed', 'Laser Engraved'] },
  { key: 'layout', label: 'Layout Support', options: ['60%', '65%', '75%', 'TKL', '96%', '1800', 'Full Size', 'Alice', 'Split', 'Ortho', '40%'] },
  { key: 'language', label: 'Language', options: ['English', 'Japanese', 'Korean', 'Minimal'] },
  { key: 'theme', label: 'Theme', options: ['Black', 'White', 'Retro', 'Cyber', 'Pastel', 'Gradient', 'Monochrome'] },
  { key: 'color', label: 'Color Family', options: ['Black', 'White', 'Grey', 'Blue', 'Green', 'Yellow', 'Red', 'Pink', 'Purple', 'Orange', 'Brown'] },
  { key: 'shineThrough', label: 'Shine Through', options: ['Yes', 'No'] },
  { key: 'compatibility', label: 'Compatibility', options: ['MX Compatible', 'Low Profile'] },
  { key: 'availability', label: 'Availability', options: ['In Stock', 'Pre Order', 'Coming Soon', 'Discontinued'] },
];
...
function getCount(key: string, draft: Record<string, string[]>): number {
...
export function KeycapFilterModal({ open, onClose, onApply, liveCount, onFilterChange }: KeycapFilterModalProps) {
  ...
  const categories: Category[] = useMemo(() => {
    const cats: Category[] = KEYCAP_CATEGORIES.map((kc) => ({
      key: kc.key,
      label: kc.label,
      count: getCount(kc.key, draft),
    }));
    if (filterData) {
      if (filterData.brands.length > 0) cats.push({ key: 'brand', label: 'Brands', count: getCount('brand', draft) });
      if (filterData.vendors.length > 0) cats.push({ key: 'vendor', label: 'Vendors', count: getCount('vendor', draft) });
    }
    cats.push({ key: 'price', label: 'Price', count: (priceMin || priceMax) ? 1 : 0 });
    return cats;
  }, [filterData, draft, priceMin, priceMax]);

  const currentGroup = useMemo(() => {
    if (activeCat === 'price') return null;
    if (activeCat === 'brand' && filterData) return { key: 'brand', label: 'Brands', options: filterData.brands.map((b) => ({ value: b.name, label: b.name })) };
    if (activeCat === 'vendor' && filterData) return { key: 'vendor', label: 'Vendors', options: filterData.vendors.map((v) => ({ value: v.name, label: v.name })) };
    const kc = KEYCAP_CATEGORIES.find((c) => c.key === activeCat);
    if (kc) return { key: kc.key, label: kc.label, options: kc.options.map((v) => ({ value: v, label: v })) };
    return null;
      ...
      <div className="fm" ref={modalRef} role="dialog" aria-modal="true">
        <div className="fm-titlebar">
          <div className="fm-titlebar-left">
            ...
            <span className="fm-titlebar-text">FILTER_KEYCAPS.sh</span>
          </div>
        ...
        <div className="fm-footer">
          ...
          <div className="fm-footer-count">{liveCount} Keycap Sets Found</div>
