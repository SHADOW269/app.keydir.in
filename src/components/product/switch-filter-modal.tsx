...
interface SwitchFilterModalProps {
  open: boolean;
  onClose: () => void;
...
const SWITCH_CATEGORIES: { key: string; label: string; options: string[] }[] = [
  { key: 'type', label: 'Switch Type', options: ['linear', 'tactile', 'clicky', 'silent', 'speed'] },
  { key: 'pin', label: 'Pin', options: ['3-pin', '5-pin'] },
  { key: 'lubing', label: 'Factory Lubing', options: ['factory_lubed', 'pre-lubed', 'unlubed'] },
  { key: 'topHousing', label: 'Top Housing', options: ['Polycarbonate', 'Nylon', 'POM'] },
  { key: 'bottomHousing', label: 'Bottom Housing', options: ['Nylon', 'POM', 'Polycarbonate'] },
  { key: 'stemMaterial', label: 'Stem Material', options: ['POM', 'UHMWPE', 'LY', 'Nylon'] },
  { key: 'springWeight', label: 'Spring Weight', options: ['35g', '45g', '50g', '55g', '60g', '67g', '70g', '80g+'] },
  { key: 'travel', label: 'Travel', options: ['Long Pole', 'Standard'] },
  { key: 'rgbCompat', label: 'RGB Compatibility', options: ['SMD', 'RGB', 'North Facing', 'South Facing'] },
...
export function SwitchFilterModal({ open, onClose, onApply, liveCount, onFilterChange }: SwitchFilterModalProps) {
  ...
  const [activeCat, setActiveCat] = useState('type');
  ...
  const categories: Category[] = useMemo(() => {
    const cats: Category[] = SWITCH_CATEGORIES.map((sc) => ({
      key: sc.key,
      label: sc.label,
      count: getCount(sc.key, draft),
    }));
    if (filterData) {
  ...
  const currentGroup = useMemo(() => {
    ...
    const sc = SWITCH_CATEGORIES.find((c) => c.key === activeCat);
    if (sc) return { key: sc.key, label: sc.label, options: sc.options.map((v) => ({ value: v, label: v })) };
    return null;
      ...
      <div className="fm" ref={modalRef} role="dialog" aria-modal="true">
        <div className="fm-titlebar">
          <div className="fm-titlebar-left">
            ...
            <span className="fm-titlebar-text">FILTER_SWITCHES.sh</span>
          </div>
        ...
        <div className="fm-footer">
          ...
          <div className="fm-footer-count">{liveCount} Switches Found</div>
